...
def detectron(INPUT_IMAGE_PATH,OUTPUT_IMAGE_PATH):
  cfg = get_cfg()
...
def generate_mask(input_image, net, palette, device = 'cuda'):
    ...
    cloth_seg = cv2.cvtColor(cv2.imread(os.path.join(cloth_seg_out_dir, 'final_seg.png')),cv2.COLOR_BGR2RGB)
...
def preprocess_images(img): # img == pth of image file
    ...
    detectron(in_detectron, out_detectron)
    device = 'cpu'
    # Create an instance of your model
    model_pre = load_seg_model(r"E:\AsadHussain\tryon\Cloth_segmentation\cloth_segm.pth", device=device)
    ...
    plt.title("uppergarment")
    plt.show()
    upper_garment_file = rf'E:\AsadHussain\tryon\StableVITON\test\gnostic-mask\{name}_mask.png'
    ...
    plt.imshow(rgb_agn)
    plt.title("rgbagnostic")
    plt.show()
    rgbagnostic_file = rf'E:\AsadHussain\tryon\StableVITON\test\gnostic-v3.2\{name}.jpg'
...
def final_model(model,save_dir=save_dir):
...
img = preprocess_images(r"E:\AsadHussain\tryon\StableVITON\test\image\00484_00.jpg")

create_txt(imagename.split('\\')[-1], '00012_00.jpg')


final_model(model, r"E:\AsadHussain\tryon\StableVITON\Output\newnewn")
