...
from detectron2.projects.DensePose2.densepose import add_densepose_config
from detectron2.projects.DensePose2.densepose.vis.extractor import DensePoseResultExtractor
from detectron2.projects.DensePose2.densepose.vis.densepose_results import DensePoseResultsFineSegmentationVisualizer as Visualizer
...
from StableVITON.cldm.plms_hacked import PLMSSampler
from StableVITON.cldm.model import create_model
from StableVITON.utils import tensor2img
...
def load_seg_model(checkpoint_path, device='cpu'):
    ...
    model_state_dict = torch.load(checkpoint_path, map_location=torch.device(device))
    new_state_dict = OrderedDict()
    for k, v in model_state_dict.items():
        name = k[7:]  # remove `module.`
        new_state_dict[name] = v
...
def load_densepose_model(cfg_path, weights_path):
    cfg = get_cfg()
...
def load_viton_model(config_path, model_path):
...
def preprocess_images(img,model_prep, predictor_densepose): # img == pth of image file
    # name = img.split('\\')[-1].split('.')[0]
    # name_ext = img.split('\\')[-1]
    
    in_detectron = img
    # out_detectron = rf'E:\AsadHussain\tryon\StableVITON\test\image-densepose\{name}.jpg'
    densepose = detectron(in_detectron, predictor_densepose)
    device = 'cpu'
    ...
    cloth_seg = generate_mask(img, net=model_prep, palette=palette, device=device)
...
def final_model(img, cloth_pth, save_dir,modelmain, model_preprocess, predictor_dense):
    print("man final model man honnnn start"*10)
    ...
    print("man final model man honnnn")
    agnostic, agnostic_mask, densepose = preprocess_images(img_pth,model_preprocess,predictor_dense)
    
    sampler = PLMSSampler(modelmain)
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        z, c = modelmain.get_input(batch, params.first_stage_key)
        bs = z.shape[0]
        c_crossattn = c["c_crossattn"][0][:bs]
        if c_crossattn.ndim == 4:
            c_crossattn = modelmain.get_learned_conditioning(c_crossattn)
            c["c_crossattn"] = [c_crossattn]
        uc_cross = modelmain.get_unconditional_conditioning(bs)
        ...
        for k, v in batch.items():
            if isinstance(v, torch.Tensor):
                ...
                #batch[k] = v
        sampler.modelmain.batch = batch
    
        ts = torch.full((1,), 999, device=z.device, dtype=torch.long)
        start_code = modelmain.q_sample(z, ts)     
        ...
        x_samples = modelmain.decode_first_stage(samples)
...
# model.eval()
# print("------------loaded ViTon model------------")

# def main_viton(image_pth, cloth_pth):
#     finalimage = final_model(image_pth, cloth_pth, r"E:\AsadHussain\tryon\StableVITON\Output\newnewn", model=model,model_pre=model_pre, predictor=predictor)
#     cv2.imwrite(r"C:\Users\asad\Desktop\idharao.jpg",finalimage)
#     return finalimage


seg_model_path = r"E:\AsadHussain\tryon\Cloth_segmentation\cloth_segm.pth"
densepose_cfg_path = r"E:\AsadHussain\tryon\detectron2\projects\DensePose2\configs\densepose_rcnn_R_50_FPN_s1x_legacy.yaml"
densepose_weights_path = r"E:\AsadHussain\tryon\detectron2\projects\DensePose2\DensePoseKeyPointsMask_ResNet50_FPN_s1x-e2e.pkl"
viton_config_path = r"E:\AsadHussain\tryon\StableVITON\configs\VITON512.yaml"
viton_model_path = r"E:\AsadHussain\tryon\StableVITON\VITONHD.ckpt"


seg_model = load_seg_model(seg_model_path)
print("------------loaded Segmentation model------------")
densepose_predictor = load_densepose_model(densepose_cfg_path, densepose_weights_path)
print("------------loaded densepose model------------")
viton_model = load_viton_model(viton_config_path, viton_model_path)
print("------------loaded viton model------------")
image = cv2.imread(image_pth)
dat = cv2.imencode('.jpg', image)[1].tobytes()
print("hellloolloolloolloo")
# finiamge = main_viton(dat, '00013_00.jpg')
fin_image = final_model(dat, '00013_00.jpg',r"E:\AsadHussain\tryon\StableVITON\Output\newnewn", viton_model, seg_model, densepose_predictor)
cv2.imwrite('newnew.jpg', fin_image)